 ...
 * @param content 内容
 */
export async function writeAndPrettify(targetFile: string, content: string) {
  createFolder(targetFile)
  fs.writeFileSync(targetFile, content)
  const prettierPath = await getPrettierPath()
...
export async function getPrettierPath() {
  const env = await getRunEnv() // 抽 到 process 中去
  let prettierPath = ''
