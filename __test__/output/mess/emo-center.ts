...
export function emocenterApiAdminJobCategoryByParentIdsGet(data: {
  isEnable?: number
  parentIds?: string[]
...
export function emocenterApiEmpowerEnterpriseReUpEntReSetEnterpriseIdPost(data: {
  enterpriseId?: string
  req?: EntReSetReq
...
export function emocenterApiEnterpriseListGet(data: {
  // 市
  city?: string
...
export function emocenterApiFeeRuleGetCooperateTypeIdsByFeeIdGet(data: {
  feeId?: string
...
export function emocenterApiSearchRecordDeleteByKeywordsPost(data: {
  groupCode?: string
  keywords?: string[]
}): Promise<ApiResponseVoid> {
...
/** 这是一个 delete 请求 */
export function emocenterApiV1TastyWorkhistoryIdDelete(data: { id?: string }): Promise<any> {
  const { id } = data
  return request.delete(`/emo-center/api/v1/tasty/work-history/${id}`)
}

/** 这是一个 put 请求 */
export function emocenterAuthExecutePut(data: any): Promise<ApiResponseVoid> {
  return request.put('/api/emo-center/auth/execute', data)
}

/** 返回数值是个数组而非对象 */
export function emocenterEffectiveDeptGet(data: {
...
/** 参数在路径上 */
export function emocenterWhiteGetWhiteIdGet(data: { whiteId?: string }): Promise<ApiResponseWhiteDetailResp> {
  const { whiteId } = data
