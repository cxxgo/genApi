...
/** 批量操作喇叭花 辟邪刀/取消辟邪刀 */
export function authDoorAuthKeyBorardsPost(data: AuthOrgKeyBorardReq): Promise<ApiResponseVoid> {
  return request.post('/api/auth/door/auth/keyBorards', data)
}

/** 辟邪刀中心 出行当前房东美丽制鞋厂团建辟邪刀简略信息甬道 */
export function authDoorAuthOrgInfosPost(data: DoorIdentificationReq): Promise<ApiResponseListAuthDoorOrgInfoResp> {
